    ...
    <div className="flex-1 flex flex-col h-full text-brand-50 overflow-hidden animate-in fade-in zoom-in-95 duration-700 rounded-3xl relative p-1">
      {region ? (
        ...
        <div className="absolute inset-0 z-50 bg-brand-900 flex flex-col animate-in fade-in zoom-in-[0.98] duration-500 rounded-[2rem] overflow-hidden">
          ...
          <div className="p-8 md:p-10 flex-none border-b border-brand-800/50 flex items-center justify-between relative overflow-hidden bg-brand-950/40 backdrop-blur-md">
             ...
             <div className="flex items-center gap-6 relative z-10">
               <button onClick={() => setSelectedRegionId(null)} title="Go Back" aria-label="Go Back" className="w-14 h-14 rounded-full bg-brand-900 border border-brand-700/50 flex items-center justify-center text-brand-300 hover:text-white hover:bg-brand-800 transition-all shadow-[0_0_20px_rgba(0,0,0,0.5)] cursor-pointer hover:-translate-x-1">
                 <ArrowLeft className="w-6 h-6" />
               </button>
               <div>
                  <h2 className="text-sm uppercase tracking-widest text-brand-400 font-medium">Region Overview</h2>
               </div>
             </div>
             ...
             <div className="relative z-10 w-20 h-20 rounded-[1.5rem] bg-brand-950/80 flex items-center justify-center text-white border border-brand-700/50 shadow-2xl">
               <region.icon className="w-10 h-10" />
             </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto scroller p-8 md:p-12 pb-32 relative">
             ...
             <div className="max-w-5xl mx-auto relative z-10 pt-4">
               ...
               <div className="flex items-center gap-3 mb-6">
                  <span className="px-4 py-2 rounded-full bg-brand-900 border border-brand-800 text-sm font-bold uppercase tracking-widest text-brand-300 flex items-center gap-2">
                     <region.tagIcon className="w-4 h-4 text-brand-primary" />
                     {region.tag}
                  </span>
               </div>
               
               <h2 className="text-5xl lg:text-7xl font-light text-white mb-10 tracking-tight leading-tight">{region.title}</h2>
               ...
               <h3 className="text-2xl font-light text-white mb-8 flex items-center gap-3 tracking-wide">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 {region.highlights.map((highlight, idx) => (
                   <div key={idx} className="bg-brand-950/40 backdrop-blur-md border border-brand-800/50 rounded-3xl p-8 hover:border-brand-primary/30 transition-colors group flex gap-6 items-start">
                     <div className="w-14 h-14 rounded-2xl bg-brand-900/80 border border-brand-800 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                       <highlight.icon className="w-7 h-7 text-brand-400 group-hover:text-brand-primary transition-colors" />
                     </div>
                     <div>
                       <h4 className="text-xl text-white font-medium mb-2">{highlight.name}</h4>
                       <p className="text-brand-300 font-light leading-relaxed">{highlight.desc}</p>
                     </div>
                   </div>
                 ))}
               </div>
        ...
        <div className="absolute inset-0 flex flex-col animate-in fade-in zoom-in-[0.98] duration-500">
          ...
          <div className="p-8 pb-6 border-b border-brand-800/50 flex-none relative z-10 bg-brand-900/40 backdrop-blur-md rounded-t-[2rem]">
            <h2 className="text-4xl font-light text-white mb-2 tracking-wide flex items-center gap-4">
              <Map className="w-10 h-10 text-brand-primary" />
              The Dalmatian Coast
            </h2>
            <p className="text-brand-300 text-sm uppercase tracking-widest">Surrounding Wonders</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-10 scroller relative z-10 space-y-10">
            {regions.map((r) => (
              <div 
                ...
                className={`relative bg-brand-950/40 backdrop-blur-xl border border-brand-800/50 rounded-[2.5rem] p-8 md:p-10 overflow-hidden group cursor-pointer transition-all duration-700 shadow-xl hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)] ${r.borderColor}`}
              >
                <div className={`absolute top-0 right-0 w-96 h-96 rounded-full blur-[100px] transition-all duration-1000 z-0 ${r.glowColor}`}></div>
                <div className="relative z-10 flex flex-col md:flex-row gap-8 lg:gap-10 items-start">
                  <div className={`w-28 h-28 lg:w-32 lg:h-32 shrink-0 rounded-[2rem] bg-brand-950 border border-brand-800 flex items-center justify-center transition-all duration-700 shadow-xl group-hover:scale-105 ${r.iconColor}`}>
                    <r.icon className="w-14 h-14 lg:w-16 lg:h-16" />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-4">
                       <span className="px-4 py-1.5 rounded-full bg-brand-900 border border-brand-800 text-xs font-bold uppercase tracking-widest text-brand-400 flex items-center gap-2">
                          <r.tagIcon className="w-3.5 h-3.5" />
                          {r.tag}
                       </span>
                    </div>
                    <h3 className="text-3xl lg:text-4xl font-light text-white mb-4 tracking-tight group-hover:text-brand-50 transition-colors">{r.title}</h3>
                    <p className="text-brand-300 leading-relaxed text-lg font-light mb-6 opacity-90 group-hover:opacity-100 transition-opacity">
                      {r.shortDescription}
                    </p>

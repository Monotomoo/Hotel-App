...
export default function EventsPane() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full text-brand-50 overflow-hidden animate-in fade-in zoom-in-95 duration-700 rounded-3xl relative">
      ...
      <div className="p-8 pb-6 border-b border-brand-800/50 flex-none relative z-10 flex justify-between items-center bg-brand-900/40 backdrop-blur-md rounded-t-[2rem]">
        <div>
          <h2 className="text-4xl font-light text-white mb-2 tracking-wide flex items-center gap-4">
             <Sparkles className="w-10 h-10 text-brand-primary" />
             Upcoming Events
          </h2>
          <p className="text-brand-300 text-sm uppercase tracking-widest">Curated Hotel Experiences</p>
        </div>
        <div className="hidden sm:flex items-center gap-3 px-6 py-3 bg-brand-950/80 shadow-inner rounded-full border border-brand-800 backdrop-blur-md">
          <Calendar className="w-5 h-5 text-brand-primary" />
          <span className="text-sm font-medium text-brand-100 uppercase tracking-widest">This Week</span>
        </div>
      </div>
      
      <div className="flex-1 overflow-y-auto p-6 md:p-10 flex flex-col gap-8 scroller relative z-10">
        
        {/* Timeline Line */}
        <div className="absolute left-10 md:left-[4.5rem] top-10 bottom-10 w-px bg-gradient-to-b from-brand-primary/80 via-brand-800 to-transparent hidden sm:block pointer-events-none z-0"></div>
        ...
        {events.map((evt) => {
          ...
          return (
            <div 
              ...
              className="group relative flex items-start gap-6 sm:gap-10 cursor-pointer w-full z-10"
            >
              
              {/* Timeline Dot & Icon */}
              <div className={`hidden sm:flex flex-none w-16 h-16 rounded-full bg-brand-950 border-2 items-center justify-center shrink-0 shadow-[0_0_20px_rgba(0,0,0,0.5)] transition-all duration-700 ease-out ${isExpanded ? 'scale-110 border-brand-primary shadow-[0_0_30px_rgba(2,132,199,0.5)]' : 'border-brand-800 group-hover:scale-110'} ${!isExpanded && evt.borderColor}`}>
                <evt.icon className={`w-8 h-8 transition-colors duration-700 ${isExpanded ? 'text-brand-primary animate-pulse' : `text-brand-400 ${evt.iconColor}`}`} />
              </div>

              {/* Event Card */}
              <div className={`flex-1 relative bg-brand-950/40 backdrop-blur-xl border border-brand-800/50 transition-all duration-700 rounded-[2.5rem] p-8 md:p-10 overflow-hidden shadow-xl ${isExpanded ? 'shadow-[0_20px_60px_rgba(2,132,199,0.2)] lg:-ml-4 border-brand-primary/50' : `hover:bg-brand-950/80 group-hover:-translate-y-2 group-hover:shadow-[0_20px_60px_rgba(0,0,0,0.5)] ${evt.borderColor}`}`}>
                ...
                <div className="relative z-10 flex flex-col xl:flex-row xl:items-start justify-between gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <h3 className="text-3xl lg:text-4xl font-light text-white mb-2 leading-tight tracking-tight">{evt.title}</h3>

                    {/* Expandable Section */}
                    <div className={`overflow-hidden transition-all duration-1000 ease-in-out ${isExpanded ? 'max-h-96 opacity-100 mt-8' : 'max-h-0 opacity-0 mt-0'}`}>
                      <div className="w-12 h-px bg-brand-500/50 mb-6"></div>
                      <p className="text-brand-200 text-lg mb-8 leading-relaxed max-w-3xl font-light">{evt.description}</p>
                      <button 
                        onClick={(e) => { e.stopPropagation(); window.location.href = `mailto:${evt.email}`; }}
                        className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-brand-primary hover:bg-white text-white hover:text-brand-900 transition-all font-medium shadow-[0_10px_30px_rgba(2,132,199,0.3)] hover:shadow-xl cursor-pointer hover:-translate-y-0.5"
                      >
